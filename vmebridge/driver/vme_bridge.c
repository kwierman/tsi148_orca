 ...
 *
 * Copyright (c) 2009 S�bastien Dugu�
 *
...
/*
 * Procfs stuff
 * Old Signature: char *page, char **start, size_t off, int count
 */
ssize_t vme_info_proc_show(struct file *filp, char __user *buf, size_t loc, loff_t * loffs)
{
    char *p = buf;
...
static struct file_operations vme_proc_fops = {
  .owner = THIS_MODULE,
  .read = vme_info_proc_show,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};

static struct file_operations window_proc_fops = {
  .owner = THIS_MODULE,
  .read = vme_window_proc_show,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};

static struct file_operations interrupts_proc_fops = {
  .owner = THIS_MODULE,
  .read = vme_interrupts_proc_show,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};

static struct file_operations irq_proc_fops = {
  .owner = THIS_MODULE,
  .read = vme_irq_proc_show,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};
...
void vme_procfs_register(void)
{
	...
	entry = proc_create("info", S_IFREG | S_IRUGO, vme_root, &vme_proc_fops);

	if (!entry)
		printk(KERN_WARNING PFX "Failed to create proc info node\n");

	/* Create /proc/vme/windows file */
	entry = proc_create("windows", S_IFREG | S_IRUGO, vme_root, &window_proc_fops);

	if (!entry)
		printk(KERN_WARNING PFX "Failed to create proc windows node\n");

	/* Create /proc/vme/interrupts file */
	entry = proc_create("interrupts", S_IFREG | S_IRUGO, vme_root, &interrupts_proc_fops);
	...
	entry = proc_create("irq", S_IFREG | S_IRUGO, vme_root, &irq_proc_fops);
 ...
 */
static  vme_bridge_create_devices(void)
{
	int i;
	int rc;
