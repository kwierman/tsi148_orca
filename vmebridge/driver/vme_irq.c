...
ssize_t vme_interrupts_proc_show(struct file *filp,char *buf,size_t count,loff_t *off)
{
	char *p = buf;
	int i;
...
//(char *page, char **start, off_t off, int count,int *eof, void *data)
ssize_t vme_irq_proc_show(struct file *filp,char *buf,size_t count,loff_t *off)
{
	char *p = buf;
	int i;
	struct vme_irq *virq;
