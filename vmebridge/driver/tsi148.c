 ...
 *(char *page, char **start, off_t off, int count,
 			  int *eof, void *data)
 */
static ssize_t tsi148_proc_pcfs_show(struct file *filp,char *buf,size_t count,loff_t *offp)
{
	char *p = buf;
	unsigned int tmp;
 ...
 *(char *page, char **start, off_t off, int count,
 			  int *eof, void *data)
 */
static ssize_t tsi148_proc_lcsr_show(struct file *filp,char *buf,size_t count,loff_t *offp)
{
	int i;
	char *p = buf;
 ...
 * (char *page, char **start, off_t off, int count,
 				 int *eof, void *data)
 */
static ssize_t tsi148_proc_gcsr_show(struct file *filp,char *buf,size_t count,loff_t *offp)
{
	char *p = buf;
	int i;
	unsigned int tmp;
 ...
 * (char *page, char **start, off_t off, int count,
 			  int *eof, void *data)
 */
ssize_t tsi148_proc_crcsr_show(struct file *filp,char *buf,size_t count,loff_t *offp)
{
	char *p = buf;
...
/**
    A whole bunch of the new proc file system tree shenangians
**/

static const struct file_operations tsi148_proc_pcfs_show_fops = {
  .owner = THIS_MODULE,
  .read = tsi148_proc_pcfs_show,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};

static const struct file_operations tsi148_proc_lcsr_show_fops = {
  .owner = THIS_MODULE,
  .read = tsi148_proc_lcsr_show,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};

static const struct file_operations tsi148_proc_gcsr_show_fops = {
  .owner = THIS_MODULE,
  .read = tsi148_proc_gcsr_show,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};

static const struct file_operations tsi148_proc_crcsr_show_fops = {
  .owner = THIS_MODULE,
  .read = tsi148_proc_crcsr_show,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};
...
void tsi148_procfs_register(struct proc_dir_entry *vme_root)
{
	...
	entry = proc_create("pcfs", S_IFREG | S_IRUGO, tsi148_proc_root, &tsi148_proc_pcfs_show_fops);
	...
	entry = proc_create("lcsr", S_IFREG | S_IRUGO, tsi148_proc_root, &tsi148_proc_lcsr_show_fops);
	...
	entry = proc_create("gcsr", S_IFREG | S_IRUGO, tsi148_proc_root, &tsi148_proc_gcsr_show_fops);
	...
	entry = proc_create("crcsr", S_IFREG | S_IRUGO, tsi148_proc_root, &tsi148_proc_crcsr_show_fops);
...
void tsi148_procfs_unregister(struct proc_dir_entry *vme_root)
{
	...
	remove_proc_entry("tsi148", vme_root);//replace tsi148 with the function call to the name
}
